...
with concurrent.futures.ThreadPoolExecutor(max_workers=12) as executor:
    ...
    for yl in range(rangey):
        for xl in range(rangex):
            ...
            future = executor.submit(download_tile, x1+(32*xl), z1+(32*yl), xl, yl)
            futures.append(future)
            done += 1
            print(f"{done} out of {todo}")
    concurrent.futures.wait(futures)
    print("all done")
for index in range(rangey):
    print(index)
    images = [Image.open(f'image/line{index}/{index}-{xl}.png') for xl in range(rangex)]
